...
**文件名使用中文**，与目录结构保持一致。专有名词文件名可使用英文 kebab-case（如框架名、工具名）。
...
- **维基/**：LLM 所有。Claude 创建、更新和维护此处的所有文件。用户仅阅读和浏览。
...
## Eric 的思考过程
[收录讨论中产生的洞察、联想、原创分析。这是 Eric 的个人思考记录，不是来源本身的内容。]
