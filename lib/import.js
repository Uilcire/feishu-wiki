...
/**
 * 按 token 修改候选条目的字段。返回 {matched, missing}。
 * tokens: string[]，patch: 要合并进每个匹配条目的字段对象。
 * missing 里是找不到对应条目的 token。
 */
function patchCandidates(candidatesPath, tokens, patch) {
  const entries = loadCandidates(candidatesPath, { strict: true });
  ...
  for (const entry of entries) {
    if (tokenSet.has(entry.token)) {
      ...
      matched.push(entry.token);
      tokenSet.delete(entry.token);
    }
  }
  for (const t of tokenSet) missing.push(t);
  if (matched.length > 0) saveCandidates(candidatesPath, entries);
  return { matched, missing };
}

/**
 * 重置候选条目的判断字段（relevant/reason/approved）回 pending 状态。
 * 不会动 imported_at / new_wiki_url —— 已导入的不重置，避免"重置后重复导入"。
 * opts.tokens: 指定 token 数组（优先）；opts.all=true: 重置所有非已导入条目。
 * 返回被影响的 token 数组。
 */
function resetCandidates(candidatesPath, { tokens = [], all = false } = {}) {
  const entries = loadCandidates(candidatesPath, { strict: true });
  const affected = [];
  const tokenSet = new Set((tokens || []).filter(Boolean));
  for (const entry of entries) {
    if (entry.imported_at) continue;
    const match = all || tokenSet.has(entry.token);
    if (!match) continue;
    entry.relevant = null;
    entry.reason = "";
  ...
  if (affected.length > 0) saveCandidates(candidatesPath, entries);
  return affected;
}

/**
 * 批量把 relevant=true 的条目标记为 approved=true。
 * 返回被影响的 token 数组。
 */
function approveAllRelevant(candidatesPath) {
  const entries = loadCandidates(candidatesPath, { strict: true });
  const affected = [];
  for (const entry of entries) {
    if (entry.relevant === true && entry.approved !== true) {
      entry.approved = true;
      affected.push(entry.token);
    }
  }
