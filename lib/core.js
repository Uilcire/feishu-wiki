...
// Cache dir: AI_WIKI_CACHE_DIR env → ~/.ai-wiki/cache/<space_id>/
function _readSpaceId() {
  try {
    const localCfg = path.resolve(".feishu-config.json");
    if (fs.existsSync(localCfg)) {
      const cfg = JSON.parse(fs.readFileSync(localCfg, "utf-8"));
      if (cfg && cfg.space_id) return cfg.space_id;
    }
    const defaultCfg = path.join(__dirname, "..", "default-config.json");
    if (fs.existsSync(defaultCfg)) {
      const cfg = JSON.parse(fs.readFileSync(defaultCfg, "utf-8"));
      if (cfg && cfg.space_id) return cfg.space_id;
    }
  } catch { /* ignore */ }
  return null;
...
function _defaultCacheDir() {
  return path.join(os.homedir(), ".ai-wiki", ".cache", _readSpaceId() || DEFAULT_SPACE_ID);
}

function _computeCacheDir() {
  if (process.env.AI_WIKI_CACHE_DIR) return process.env.AI_WIKI_CACHE_DIR;
  // Legacy fallback: if cwd has a .cache/ dir, use it (will be migrated by ensureCache)
  const cwdCache = path.resolve(".cache");
  try {
    if (fs.existsSync(cwdCache) && fs.statSync(cwdCache).isDirectory()) return cwdCache;
  } catch { /* ignore */ }
  return _defaultCacheDir();
}

function _recomputePaths() {
  CACHE_DIR = _computeCacheDir();
  BUILDING_FILE = path.join(CACHE_DIR, ".building");
  INDEX_FILE = path.join(CACHE_DIR, "index.json");
  STATE_FILE = path.join(CACHE_DIR, "state.json");
  LOG_FILE = path.join(CACHE_DIR, "日志.md");
  DOCS_DIR = path.join(CACHE_DIR, "docs");
  QA_QUEUE_FILE = path.join(CACHE_DIR, "qa-events.ndjson");
}

let CACHE_DIR, BUILDING_FILE, INDEX_FILE, STATE_FILE, LOG_FILE, DOCS_DIR, QA_QUEUE_FILE;
_recomputePaths();
...
// === 读操作元数据（供 commands.js 读取）===

let _lastIndexMeta = { freshness: "cached", data_source: "index" };
let _lastFetchMeta = { freshness: "cached", data_source: "disk_cache" };
...
let _qaSequenceNumber = 0;

function _logQaEvent(eventType, input, outputSummary, extras) {
  if (!QA_LOG_ENABLED) return;
  const user = lark.currentUser();
  const entry = {
    session_id: QA_SESSION_ID,
    sequence_number: ++_qaSequenceNumber,
    user_name: user.name,
...
function refreshIndexIfStale() {
  ...
  if (fs.existsSync(INDEX_FILE) && now - _indexLastRefresh < INDEX_TTL) {
    ...
    if (cached) {
      ...
      _lastIndexMeta = { freshness: "cached", data_source: "index" };
      return cached;
    }
  ...
  if (fs.existsSync(INDEX_FILE)) {
    try {
      ...
      saveState(state);
      _lastIndexMeta = { freshness: "fresh", data_source: "index" };
      return index;
    } catch (err) {
      ...
      if (cached) {
        ...
        saveState(state);
        _lastIndexMeta = { freshness: "stale-fallback", data_source: "index" };
        process.stderr.write(
  ...
  // Check if background build is in progress
  if (isBuilding()) {
    _lastIndexMeta = { freshness: "building", data_source: "index" };
  } else {
    _lastIndexMeta = { freshness: "fresh", data_source: "index" };
  }
...
function spawnBackgroundBuild() {
  ...
  const child = spawn(process.execPath, [script], {
    ...
    detached: true,
    stdio: "ignore",
    env: { ...process.env, AI_WIKI_CACHE_DIR: CACHE_DIR },
  });
  child.unref();
...
function ensureCache() {
  ...
  // Migrate: if CACHE_DIR is the legacy cwd-relative .cache/, delete it and
  // switch to the new home-dir path so the fresh build lands there.
  const homeCacheDir = _defaultCacheDir();
  const cwdCache = path.resolve(".cache");
  if (CACHE_DIR === cwdCache && cwdCache !== homeCacheDir) {
    try {
      fs.rmSync(cwdCache, { recursive: true, force: true });
      process.stderr.write(`[fw] 已迁移缓存: ${cwdCache} → ${homeCacheDir}\n`);
    } catch { /* best-effort cleanup */ }
    _recomputePaths(); // .cache/ gone → falls through to home dir
  }
...
function find(query, { category = null, includeDeprecated = false } = {}) {
  ...
  // 收集所有候选（用于 ambiguity_count / top_candidates）
  const allCandidates = [];
  ...
  if (pages[query]) {
    ...
    if (!includeDeprecated && page.deprecated) {
      ...
      exactMatch = { ...page, title: query, match_type: "exact" };
    }
  }

  // 模糊匹配（总是收集，用于 ambiguity 统计）
  const fuzzyMatches = [];
  for (const [title, info] of Object.entries(pages)) {
    if (!includeDeprecated && info.deprecated) continue;
    if (category !== null && info.category !== category) continue;
    if (title === query) continue; // 精确匹配已单独处理
    if (title.includes(query) || query.includes(title)) {
      fuzzyMatches.push({ ...info, title, match_type: "fuzzy" });
    }
  }
  fuzzyMatches.sort((a, b) => a.title.length - b.title.length);
  ...
  // 确定最佳匹配
  const best = exactMatch || (fuzzyMatches.length ? fuzzyMatches[0] : null);
  if (!best) {
  ...
  // 构建 top_candidates（排除 best 自身，最多 5 个）
  const topCandidates = allCandidates
    .filter((c) => c.title !== best.title)
    .slice(0, 5)
    .map((c) => ({ title: c.title, category: c.category || "", match_type: c.match_type }));

  best.ambiguity_count = allCandidates.length;
  best.top_candidates = topCandidates;
  best.freshness = _lastIndexMeta.freshness;
  best.data_source = _lastIndexMeta.data_source;

  _logQaEvent("call:find", query, best.title, {
    latency_ms: Date.now() - t0,
    outcome: best.match_type === "exact" ? "success" : "partial",
  });
  return best;
}
...
function fetch(pageOrTitle, { fresh = false } = {}) {
  ...
  if (!fresh && fs.existsSync(cachePath)) {
    if (cachedTimes[title] === (pageInfo.obj_edit_time || "")) {
      ...
      _lastFetchMeta = { freshness: "cached", data_source: "disk_cache" };
      _logQaEvent("call:fetch", title, md.slice(0, 200), {
        latency_ms: Date.now() - t0,
        outcome: "cached",
        payload_size_bytes: Buffer.byteLength(md, "utf-8"),
      });
      return md;
    }
  }
  ...
  _lastFetchMeta = { freshness: "fresh", data_source: "remote_api" };
  _logQaEvent("call:fetch", title, md.slice(0, 200), {
    latency_ms: Date.now() - t0,
    outcome: "success",
    payload_size_bytes: Buffer.byteLength(md, "utf-8"),
...
function fetchHead(pageOrTitle, { fresh = false } = {}) {
  ensureCache();
  ...
  // 获取 markdown（用于提取 sections）
  const md = fetch(title, { fresh });
  const sections = _extractSections(md);

  _logQaEvent("call:fetchHead", title, `sections=${sections.length}`);
  return {
    title,
    category: pageInfo.category || null,
    summary: pageInfo.summary || "",
    obj_edit_time: pageInfo.obj_edit_time || "",
    deprecated: Boolean(pageInfo.deprecated),
    url: pageInfo.url || "",
    sections,
  };
}

function fetchSection(pageOrTitle, sectionName, { fresh = false } = {}) {
  ensureCache();
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (startIdx === -1 && heading.includes(needle)) {
      startIdx = i;
    } else if (startIdx !== -1) {
      endIdx = i;
      break;
  ...
  if (startIdx === -1) {
    const available = _extractSections(md);
    const msg = available.length
      ? `找不到章节 "${sectionName}"。可用章节: ${available.join(", ")}`
      : `找不到章节 "${sectionName}"。该页面没有 H2 章节。`;
    throw new Error(msg);
  }

  const section = lines.slice(startIdx, endIdx).join("\n").trimEnd();
  _logQaEvent("call:fetchSection", `${title}#${sectionName}`, section.slice(0, 200));
  return section;
}

function fetchExcerpt(pageOrTitle, keyword, { fresh = false, window: windowSize = 5 } = {}) {
  ensureCache();
  ...
  if (matchIndices.length === 0) {
    throw new Error(`在页面 "${title}" 中未找到关键词 "${keyword}"`);
  }
  ...
  for (const idx of matchIndices) {
    ...
    if (ranges.length > 0 && start <= ranges[ranges.length - 1].end + 1) {
      ...
      ranges.push({ start, end, matches: [idx] });
    }
  }
  ...
  for (const range of ranges) {
    ...
    for (let i = range.start; i <= range.end; i++) {
      const prefix = range.matches.includes(i) ? ">" : " ";
      block.push(`${prefix} ${i + 1}: ${lines[i]}`);
    }
    blocks.push(block.join("\n"));
  }

  const result = blocks.join("\n---\n");
  _logQaEvent("call:fetchExcerpt", `${title}:${keyword}`, `${matchIndices.length} matches`);
  return result;
}
...
function _checkWritePermission(titleOrCommand) {
  ...
  if (fs.existsSync(configPath)) {
    try {
      ...
      if (!cfg.write_enabled) {
        _logQaEvent("denied:write", titleOrCommand || "", "write_enabled=false", {
          outcome: "denied",
      ...
      // Malformed config → safe default: deny writes
      _logQaEvent("denied:write", titleOrCommand || "", "config_parse_error", {
        outcome: "denied",
        error_type: "validation_error",
      });
      throw new Error(
        "配置文件解析失败，默认拒绝写入。\n" +
        "请检查 ~/.feishu-wiki-config.json 格式，或运行：ai-wiki mode write"
      );
    }
  }
}
...
function create(category, title, content, { summary = "", force = false } = {}) {
  const lock = require("./lock");
  _checkWritePermission(title);
  ensureCache();
  ...
  if (!force) {
    ...
    if (existing) {
      throw new Error(
        `页面「${title}」已存在于 ${existing.category}，使用 update 命令修改现有页面`
      );
    }
  }
...
function update(pageOrTitle, content, { mode = "append", summary = "", force = false } = {}) {
  const lock = require("./lock");
  const title =
  ...
  function doUpdate() {
    ...
    if (!force && page.category && page.category.startsWith("原始资料")) {
      throw new Error(
        `原始资料页面「${title}」不可修改（原始资料创建后不可变）`
      );
    }
...
function del(pageOrTitle, { reason = "", force = false } = {}) {
  const lock = require("./lock");
  const title =
    typeof pageOrTitle === "object" && pageOrTitle !== null
      ? pageOrTitle.title || ""
...
function verifyWrite(title) {
  ensureCache();
  const index = refreshIndexIfStale();
  const pageInfo = (index.pages || {})[title];
  if (!pageInfo) {
    const found = find(title);
    if (!found) {
      return {
  ...
  let content;
  try {
    content = fetch(title, { fresh: true });
  } catch {
    return {
      ok: false,
      title,
  ...
  while ((m = wikiRe.exec(content))) {
    ...
    if (!seenWiki.has(raw)) {
      seenWiki.add(raw);
      unresolvedWikilinks.push(raw);
    }
  }

  // Check for <mention-doc> tokens that don't match any page in index
  const brokenMentions = [];
  const tokenMap = {};
  for (const [t, info] of Object.entries(index.pages || {})) {
    if (info.obj_token) tokenMap[info.obj_token] = t;
  }
  const mentionRe = /<mention-doc[^>]*token="([^"]+)"[^>]*>([^<]*)<\/mention-doc>/g;
  while ((m = mentionRe.exec(content))) {
    const token = m[1];
    if (!tokenMap[token]) {
      brokenMentions.push(`<mention-doc token="${token}">${m[2]}</mention-doc>`);
    }
  }

  // Check for attribution callout
  const hasAttribution = CALLOUT_RE.test(content);

  // Check for sources section (来源 category pages should link to 原始资料)
  const resolvedPage = (index.pages || {})[title] || {};
  let hasSourcesSection = true; // default true for non-来源 pages
  if (resolvedPage.category === "来源") {
    // 来源 pages should have a section linking to raw materials
    hasSourcesSection =
      content.includes("原始资料") || content.includes("来源");
  }

  const ok =
    contentNonempty &&
    unresolvedWikilinks.length === 0 &&
    brokenMentions.length === 0 &&
    hasAttribution &&
    hasSourcesSection;
  ...
  return {
    ...
    checks: {
      page_exists: true,
      content_nonempty: contentNonempty,
      unresolved_wikilinks: unresolvedWikilinks,
      broken_mentions: brokenMentions,
...
module.exports = {
  ...
  _invalidateCache() { _cachedIndex = null; _configCache = undefined; _recomputePaths(); },
  get CACHE_DIR() { return CACHE_DIR; },
  get BUILDING_FILE() { return BUILDING_FILE; },
  get DOCS_DIR() { return DOCS_DIR; },
  // 读操作元数据（供 commands.js 读取）
  get lastFetchMeta() { return _lastFetchMeta; },
  get lastIndexMeta() { return _lastIndexMeta; },
};
