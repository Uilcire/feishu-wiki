...
function lint() {
  ...
  // 一次性构建所有数据结构，避免重复调用 listPages / refreshIndexIfStale
  const pages = [];
  const allWithDep = [];
  for (const [title, info] of Object.entries(allPagesObj)) {
    const entry = { title, ...info };
    allWithDep.push(entry);
    if (!info.deprecated) pages.push(entry);
  }
