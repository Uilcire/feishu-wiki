...
function grep(pattern, { category = null, ignoreCase = true } = {}) {
  ...
  const allPages = core.listPages({ category });
  const index = core.loadIndex();
  const totalPagesIndexed = Object.keys((index && index.pages) || {}).length;

  let cachedDocsScanned = 0;
  const results = [];
  ...
  const indexMeta = core.lastIndexMeta || { freshness: "cached", data_source: "index" };

  return {
    results,
    coverage: {
      cached_docs_scanned: cachedDocsScanned,
      total_pages_indexed: totalPagesIndexed,
      coverage_ratio: totalPagesIndexed > 0
        ? Math.round((cachedDocsScanned / totalPagesIndexed) * 100) / 100
        : 0,
    },
    freshness: indexMeta.freshness,
...
function searchFeishu(keyword, { limit = 10, allDocs = false } = {}) {
  ...
  // 获取 wiki 的 obj_tokens 用于过滤
  // 优先用本地索引；索引未就绪时从云端根页面拉取 token 清单
  let wikiTokens = null;
  ...
  if (index && Object.keys(index.pages || {}).length > 0) {
    ...
    for (const info of Object.values(index.pages)) {
      if (info.obj_token) wikiTokens.add(info.obj_token);
    }
    tokenSource = "index";
  } else {
    wikiTokens = core.loadWikiTokensFromCloud(); // returns Set or null
    tokenSource = wikiTokens ? "cloud_tokens" : null;
  }
