...
function runUpgrade(flags) {
  ...
  try {
    ...
    // 升级后自动 reset 未导入的 import 候选：SKILL 规则/主题常随版本调整，
    // 旧版判断（relevant/reason/approved）会失效，留着反而误导 Agent。
    // 已导入的条目不动（resetCandidates 内部自己保护 imported_at）。
    try {
      const cfg = loadImportConfig();
      const cpath = cfg && cfg.import && cfg.import.candidates_path;
      if (cpath) {
        const imp = require("./import");
        const fs = require("fs");
        const full = imp.expandHome(cpath);
        if (fs.existsSync(full)) {
          const affected = imp.resetCandidates(cpath, { all: true });
          if (affected.length > 0) {
            console.log(`  ♻️  import 候选已 reset (${affected.length} 条回 pending) —— 请重新走 scan/mark/approve 流程`);
          }
        }
      }
    } catch (e) {
      process.stderr.write(`[fw] 升级后 reset 候选失败（可忽略）：${e.message || e}\n`);
    }
