...
function runSetup() {
  ...
  try {
    ...
    const scopes = "drive:drive drive:drive:readonly space:document:retrieve wiki:wiki wiki:node:move";
    try {
...
function handleImport(positional, flags) {
  ...
  if (!sub) {
    console.error("用法: ai-wiki import <scan|list|apply> [...]");
    console.error("  import scan <folder_token>              扫描云空间文件夹，合并到候选清单");
  ...
  if (sub === "scan") {
    ...
    if (!folderToken) {
      console.error("用法: ai-wiki import scan <folder_token>");
      process.exit(1);
    }
  ...
  if (sub === "apply") {
    ...
    // dry-run 不产生副作用，允许在学习模式下执行以便 Agent 预演。
...
function showHelp() {
  ...
  console.log();
  console.log("导入:");
  console.log("  import scan <folder_token>               扫描云空间文件夹，合并到候选清单");
