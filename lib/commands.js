...
function main(argv) {
  ...
  switch (cmd) {
    ...
    case "grep": {
      ...
      json(search.grep(query, { category: flags.category || null }));
      break;
    }
    ...
    case "search": {
      ...
      const search = require("./search");
      json(search.searchFeishu(query, { allDocs: "all-docs" in flags }));
      break;
    }
...
function runSetup() {
  ...
  try {
    ...
    try {
      execFileSync("lark-cli", ["auth", "login"], { stdio: "inherit" });
      console.log("  ✅ 登录成功");
    } catch {
...
function runUpgrade(flags) {
  ...
  let latest;
  try {
    latest = execFileSync("npm", ["view", "ai-wiki", "version"], {
      encoding: "utf-8",
      timeout: 15000,
    ...
    console.error("  手动更新：npm install -g ai-wiki@latest");
    process.exit(1);
  }
  ...
  try {
    execFileSync("npm", ["install", "-g", "ai-wiki@latest"], {
      stdio: "inherit",
      timeout: 60000,
    });
    console.log();
